  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-white rounded-xl border border-gray-100 hover:shadow-md transition-shadow duration-200">
      ...
      <div className="min-w-0">
        <p className="font-semibold text-[#0B2240] text-sm leading-tight truncate">{member.name}</p>
        <p className="text-xs text-slate-500 uppercase tracking-wide mt-0.5 truncate">{member.designation}</p>
      </div>
    </div>
  );
}
...
export default async function ActiveBoardPage() {
  ...
  const members = await getActiveBoard(currentYear);

  // Tier 1: Club President
  const president = members.find(
    (m) => m.designation === "Club President" || m.designation === "President"
  );
  ...
  // Board poster image — from MongoDB, falling back to static file
  let boardImageSrc = "/board-poster.jpeg";
  try {
    const db = await getDb();
    const boardImg = await db
      .collection("boardImages")
      .findOne({ _type: "boardOfDirectors" });
    if (boardImg?.imageUrl) boardImageSrc = boardImg.imageUrl;
  } catch {
    // Fallback to static file on any DB error
  }
      ...
      <section className="bg-white border-b border-gray-100 pt-20 pb-12 px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="mt-3 text-slate-500 max-w-xl">
            ...
            <span className="font-semibold text-[#0B2240]">Rotary Club of Amravati Ambika</span>
            {" "}for FY&nbsp;{currentYear}.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto px-6 py-16 space-y-20">
        ...
        {president && (
          <section className="flex flex-col items-center">
            ...
            <Link href={`/past-presidents/${president.id}`} className="group max-w-sm w-full">
              <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 text-center cursor-pointer">
                <div className="relative h-80 bg-slate-100 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-0 right-0 px-6 text-white text-left">
                    <h2 className="text-2xl font-extrabold leading-tight">{president.name}</h2>
        ...
        {secretaries.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
              {secretaries.map((sec) => (
                <div key={sec.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300 text-center">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-xs text-slate-500 uppercase tracking-wider mt-1 font-semibold">{sec.designation}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {roster.length > 0 && (
          <section>
            <div className="mb-8">
              <h2 className="text-2xl font-extrabold text-[#0B2240]">Extended Board of Directors</h2>
              <p className="text-slate-500 text-sm mt-1">Complete roster for FY {currentYear}.</p>
            </div>
        ...
        {/* Board Poster — fetched from MongoDB or fallback to static file */}
